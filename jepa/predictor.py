...
class JEPPredictor(nn.Module):
    ...
    针对 MSL 数据集优化：
    - 输入: (B, 9, 128) - 9个历史patch的latent
    - 输出: (B, 9, 128) - 9个未来patch的latent预测
    """
    def __init__(
        self,
        latent_dim: int = 128,        # 与Encoder保持一致
        num_heads: int = 4,           # 128/4=32，保证维度能整除
        num_layers: int = 3,          # 减少到3层，减轻计算负担
        ffn_dim: int = None,
        dropout: float = 0.1,
        prediction_length: int = 9     # MSL: 预测9个patch
    ):
        """
        Args:
            latent_dim:        Encoder 输出的 latent 维度 D (128)
            num_heads:         注意力头数 (4)
            num_layers:        Transformer Encoder/Decoder 层数 (3)
            ffn_dim:          Feed‐forward 网络维度，默认 4 * latent_dim
            dropout:          dropout 比例
            prediction_length: 预测的 patch 数 N_tgt (9)
        """
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ctx_latents: (B, 9, 128) - 9个历史patch的latent
            tgt_latents: (B, 9, 128) - 9个未来patch的latent，训练时需要
        Returns:
            如果 tgt_latents 不为 None（训练）：
                pred_latents:  (B, 9, 128)
        ...
        ctx = self.pos_encoding(ctx_latents)  # (B, 9, 128)
        ...
        if tgt_latents is not None:
            ...
            decoder_input = torch.cat([sos, tgt_latents[:, :-1, :]], dim=1)  # (B, 9, 128)

            # 3) 给 decoder_input 加位置编码
            dec = self.pos_encoding(decoder_input)  # (B, 9, 128)

            # 4) 构造 causal mask
            tgt_mask = self._generate_square_subsequent_mask(N_tgt, device)  # (9, 9)

            # 5) 输入 Transformer
            #    src: (B, 9, 128), tgt: (B, 9, 128)
            ...
            pred_latents = self.projection(output)  # (B, 9, 128)
            ...
            # —— 推理模式：自回归生成 9 个 latent
            assert self.pred_len is not None, "推理时必须提供 prediction_length"
            N_tgt = self.pred_len

            preds = []
            # 以一个全零 <sos> 开始
            prev = torch.zeros(B, 1, D, device=device)  # (B, 1, 128)

            for t in range(N_tgt):
                # 1) 拼接上一步生成的 latent（或 <sos>）
                prev_pe = self.pos_encoding(prev)  # (B, t+1, 128)
                ...
                out = self.transformer(
                    src=ctx,         # (B, 9, 128)
                    tgt=prev_pe,     # (B, t+1, 128)
                    tgt_mask=tgt_mask
                )  # out: (B, t+1, 128)

                # 4) 取出最后一个位置的隐藏向量，投影到下一个 latent
                next_latent = self.projection(out[:, -1:, :])  # (B, 1, 128)
                preds.append(next_latent)

                # 5) 把 next_latent 拼回 prev，用于下一步生成
                prev = torch.cat([prev, next_latent], dim=1)  # (B, t+2, 128)
