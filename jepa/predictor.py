...
class JEPPredictor(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            dropout:           dropout 比例
            prediction_length: 预测的 patch 数 N_tgt（推理时需要）
        """
        super().__init__()
    ...
    def forward(
        ...
        if tgt_latents is not None:
            ...
            decoder_input = torch.cat([sos, tgt_latents[:, :-1, :]], dim=1)  # (B, N_tgt, D)

            # 3) 给 decoder_input 加位置编码
            dec = self.pos_encoding(decoder_input)  # (B, N_tgt, D)

            # 4) 构造 causal mask
            tgt_mask = self._generate_square_subsequent_mask(N_tgt, device)  # (N_tgt, N_tgt)

            # 5) 输入 Transformer
            #    src: (B, N_ctx, D), tgt: (B, N_tgt, D)
            ...
            pred_latents = self.projection(output)  # (B, N_tgt, D)
