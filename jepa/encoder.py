...
class MyTimeSeriesEncoder(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            patch_length: 每个 patch 的时间步数 T
            num_vars: 每个时间步的特征维度 F
            latent_dim: 最终输出的 patch-level latent 维度 D
            time_layers: patch 内部时间 Transformer 层数
            patch_layers: patch 级别 Transformer 层数
        ...
        # —— 第一阶段：patch 内部时序编码
        # 1) 1D Conv 将 (F → hidden_dim)
        self.hidden_dim = latent_dim // 2  # 例如 256 → 128
        ...
        # 4) 把 (T, hidden_dim) 平均池化或最大池化成 (hidden_dim)
        #    然后映射到 latent_dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: Tensor, shape = (B, N, T, F)
        Returns:
            Tensor, shape = (B, N, latent_dim)
        """
        B, N, T, F = x.shape
        assert F == self.num_vars, f"输入特征维度 {F} 与 num_vars {self.num_vars} 不匹配"

        # —— Stage 1: patch 内部
        # 把每个 patch 展成 (B*N, F, T)，做 1D Conv
        x_ = x.view(B * N, T, F).transpose(1, 2)  # (B*N, F, T)
        h = self.act(self.conv1(x_))             # (B*N, hidden_dim, T)

        # 转置到 (B*N, T, hidden_dim)，加时间位置编码
        h = h.transpose(1, 2) + self.time_pos_emb.unsqueeze(0)  # (B*N, T, hidden_dim)

        # 通过时间 Transformer，捕捉 patch 内部时序依赖
        h = self.transformer_time(h)  # (B*N, T, hidden_dim)

        # 这里用平均池化得到 (B*N, hidden_dim)
        h_pooled = h.mean(dim=1)       # (B*N, hidden_dim)

        # 映射到 latent_dim
        patch_latent = self.fc_time2latent(h_pooled)  # (B*N, latent_dim)
        patch_latent = patch_latent.view(B, N, self.latent_dim)  # (B, N, latent_dim)

        # —— Stage 2: patch 级别
        # 1) 加 Patch 位置编码
        patch_latent = self.patch_pos_encoding(patch_latent)  # (B, N, latent_dim)

        # 2) 通过 patch‐level Transformer
        y = self.transformer_patch(patch_latent)  # (B, N, latent_dim)

        # 3) 最后归一化
        y = self.norm(y)  # (B, N, latent_dim)
        return y
